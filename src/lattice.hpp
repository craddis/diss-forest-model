...
struct lattice { // Complete Hexagonal Lattice covering a rectangular area	
	...
	int rwalk(int snake, int nsteps) { // NO FORMAL TESTING
		for(int i=0; i!= nsteps; ++i) snake = *one_of( adjlist.find(snake)->second );
		return snake;
	}	
	/**array<double, 2> xy(int snake) { // UNCOMPILED
		int q = snake % ncol;
		int r = snake / ncol;
		int odd = r % 2;
	...
	/**double azimuth(int origin, int dest) {  // UNCOMPILED
		auto oxy = xy(origin);
		auto dxy = xy(dest);
		double dx = dxy[0]-oxy[0];
		double dy = dxy[1]-oxy[1];
		double theta = cmath.arctan(dy/dx);
